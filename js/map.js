...
function initMap() {
  ...
  window.addMarkerToMap = function(report) {
    ...
    marker.bindPopup(`<div style="min-width:200px;padding:8px"><div style="font-size:18px;font-weight:700">${categoryIcons[report.category]||'📍'} ${LSCW.sanitise(report.category)}</div><p style="font-size:12px">${LSCW.sanitise(report.description).substring(0,100)}...</p><p style="font-size:10px;color:#94a3b8">${report.reportId} | ${report.status}</p></div>`, { maxWidth: 260 });
    return marker;
  };
