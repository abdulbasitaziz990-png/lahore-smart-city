...
def resend_otp(request):
    ...
    return Response({'message': 'New OTP sent', 'dev_otp': otp})
...
def list_users(request):
    ...
    data = [{'id': u.id, 'name': u.name, 'email': u.email, 'phone': u.phone, 'role': u.role, 'is_verified': u.is_verified, 'created_at': u.created_at, 'reports_count': u.reports.count()} for u in users]
...
def forgot_password(request):
    ...
    reset_otp_store[email] = {'otp': otp, 'expires_at': timezone.now() + timedelta(minutes=10)}
    print(f"[DEV] Password Reset OTP for {email}: {otp}")
    return Response({'message': 'OTP sent to your email', 'dev_otp': otp}, status=status.HTTP_200_OK)
...
def verify_reset_otp(request):
    ...
    return Response({'message': 'OTP verified', 'reset_token': reset_token}, status=status.HTTP_200_OK)
