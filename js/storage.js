...
async function registerUser({ name, email, phone, password }) {
    ...
    const data = await res.json();
    if (data.message) return { ok: true, email: data.email, devOtp: data.dev_otp };
...
function simulateSendOTP(email) { const otp = Math.floor(100000 + Math.random() * 900000).toString(); console.log(`[DEV] OTP for ${email}: ${otp}`); return otp; }
function verifyOTP(email, otp) { return { ok: true }; }
function markUserVerified(email) { return true; }
...
function saveReport(report) { 
    ...
    notifications.push({
        ...
        userId: 'admin-001',
        title: 'New Report',
        message: `${report.category} reported by ${report.reportedBy}`,
        type: 'new_report',
        reportId: report.id,
        read: false,
...
function updateReportStatus(id, status, data = {}) { 
    ...
    if (idx >= 0) { 
        ...
        if (reports[idx].reporterId) {
            ...
            notifications.push({
                id: Date.now().toString(36),
                userId: reports[idx].reporterId,
                title: 'Status Updated',
                message: `Your ${reports[idx].category} report is now ${status}`,
                type: 'status_update',
                reportId: id,
                read: false,
                createdAt: new Date().toISOString()
...
function getNotifications(userId) { 
    const all = JSON.parse(localStorage.getItem('lscw_notifications') || '[]');
    return all.filter(n => n.userId === userId && !n.read).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
}
...
function markAllNotificationsRead(userId) { 
    ...
    localStorage.setItem('lscw_notifications', JSON.stringify(all));
}
function markNotificationRead(id) {
    const all = JSON.parse(localStorage.getItem('lscw_notifications') || '[]');
    const idx = all.findIndex(n => n.id === id);
    if (idx >= 0) { all[idx].read = true; localStorage.setItem('lscw_notifications', JSON.stringify(all)); }
}
function createNotification(data) {
    const all = JSON.parse(localStorage.getItem('lscw_notifications') || '[]');
    all.push({ id: Date.now().toString(36), ...data, read: false, createdAt: new Date().toISOString() });
    localStorage.setItem('lscw_notifications', JSON.stringify(all));
}
...
function createTicket(data) { 
    ...
    const ticket = { id: Date.now().toString(36), ticketNumber: 'SUP-' + Date.now().toString().slice(-6), ...data, status: 'open', messages: [{ id: Date.now().toString(36), userId: data.userId, userName: data.userName, message: data.message, isAdmin: false, createdAt: new Date().toISOString() }], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    tickets.push(ticket);
    localStorage.setItem('lscw_tickets', JSON.stringify(tickets));
    return { ok: true, ticket };
}
function addTicketReply(id, uid, name, msg, isAdmin) { 
    const tickets = getTickets();
    const idx = tickets.findIndex(t => t.id === id);
    if (idx >= 0) { tickets[idx].messages.push({ id: Date.now().toString(36), userId: uid, userName: name, message: msg, isAdmin, createdAt: new Date().toISOString() }); tickets[idx].updatedAt = new Date().toISOString(); tickets[idx].status = isAdmin ? 'replied' : 'awaiting_reply'; localStorage.setItem('lscw_tickets', JSON.stringify(tickets)); return true; }
    return false;
}
