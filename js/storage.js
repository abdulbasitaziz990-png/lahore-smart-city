...
function markNotificationRead(id) { const all = JSON.parse(localStorage.getItem('lscw_notifications') || '[]'); const idx = all.findIndex(n => n.id === id); if (idx >= 0) { all[idx].read = true; localStorage.setItem('lscw_notifications', JSON.stringify(all)); } }
function createNotification(data) { const all = JSON.parse(localStorage.getItem('lscw_notifications') || '[]'); all.push({ id: Date.now().toString(36), ...data, read: false, createdAt: new Date().toISOString() }); localStorage.setItem('lscw_notifications', JSON.stringify(all)); }
